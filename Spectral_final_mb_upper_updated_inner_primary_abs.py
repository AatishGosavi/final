...
def save_spectral_data_to_db(data):
    ...
    try:
        ...
        set_str = ", ".join(set_clauses) if set_clauses else "bobbin_no = EXCLUDED.bobbin_no"

        upsert_query = f"""
            INSERT INTO qc_entry_temp ({", ".join(columns)}) 
            VALUES ({", ".join(placeholders)})
            ON CONFLICT (bobbin_no) 
                    ...
                    if current_section == "mbend":
                        ...
                        if mbend_data["wavelength_cols"]:
                            ...
                            for i, wl in enumerate(mbend_data["wavelength_cols"]):
                                idx = 4 + i
                                if idx < len(parts):
                                    atten_val = safe_python_numeric(parts[idx])
                                    # CHANGED: force attenuation value to always be positive,
                                    # even if the source file reports it as a negative value.
                                    if atten_val is not None:
                                        atten_val = abs(atten_val)
                                    values[wl] = atten_val
