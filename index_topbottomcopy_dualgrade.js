...
async function validateBobbinQC(bobbinNo) {
  ...
  try {
    ...
    // --- NEW LOGIC: Check pt_entry.full_test to decide whether top/bottom copy should run at all.
    // full_test = true  -> this bobbin was fully tested (both top & bottom present as-is), skip copy
    // full_test = false -> allow borrowing missing top/bottom values from whichever side is present
    const fullTestQuery = `SELECT full_test FROM pt_entry WHERE bobbin_no = $1;`;
    const fullTestRes = await client.query(fullTestQuery, [bobbinNo]);
    const fullTest = fullTestRes.rows.length > 0 ? fullTestRes.rows[0].full_test === true : false;
    // --------------------------------------------------------------------------
    ...
    // --- NEW LOGIC: Special case - if matcode is 'D' and mac_value < 7.05, pull specs for the
    // original matcode AND a secondary matcode (its tiers appended after), instead of just one.
    // NOTE: 'D2' below is a placeholder - update SECONDARY_MATCODE_FOR_LOW_MAC to the real value. ---
    const SECONDARY_MATCODE_FOR_LOW_MAC = 'D2';

    const macValueParsed = parseFloat(measurement['mac_value']);
    const useDualMatcodeSpecs = (matcode === 'D' && !isNaN(macValueParsed) && macValueParsed < 7.05);

    let specsRes;
    if (useDualMatcodeSpecs) {
      // Result order: matcode='D' tiers first (priority 1,2,3...), then secondary matcode's tiers (priority 1,2,3...)
      const matcodesInOrder = [matcode, SECONDARY_MATCODE_FOR_LOW_MAC];
      const dualSpecsQuery = `
        SELECT * FROM qc_grade 
        WHERE Matcode = ANY($1) AND Status = true 
        ORDER BY array_position($1, Matcode), priority ASC;
      `;
      specsRes = await client.query(dualSpecsQuery, [matcodesInOrder]);
    } else {
      const specsQuery = `
        SELECT * FROM qc_grade 
      ...
      `;
      specsRes = await client.query(specsQuery, [matcode]);
    }
    // --------------------------------------------------------------------------
    ...
    // NOTE: 'cable_cut_off' is handled separately below (its own conditional mandatory rule),
    // so it is excluded from this generic blanket check. ---
    const missingParameters = [];
    for (const paramName of parametersToCheck) {
      if (paramName === 'cable_cut_off') continue;
